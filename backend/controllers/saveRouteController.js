...
const saveRoute = async (req, res) => {
  try {
    ...
    for await (const segment of route.segments) {
      const routeDetails = await saveRouteModel.saveRouteDetails(
        segment,
        routeInfo.routeid,
    ...
    const routeStatus = await saveRouteModel.saveRouteStatus(
      routeInfo.routeid,
      allRouteDetails[0].routedetailid,
      allRouteDetails[0].statusid,
      route.lastUpdatedUserId,
    );
...
const updateRoute = async (req, res) => {
  try {
    ...
    let i = 0;
    for await (const segment of route.segments) {
      let routeDetails = await saveRouteModel.updateRouteDetails(
        routeInfo.routeid,
        i,
        segment,
        route.lastUpdatedUserId,
      ...
      if (routeDetails === undefined) {
        routeDetails = await saveRouteModel.saveRouteDetails(
          segment,
          routeInfo.routeid,
          route.lastUpdatedUserId,
          i,
    ...
    const routeStatus = await saveRouteModel.updateRouteStatus(
      route.routeStatusId,
      routeInfo.routeid,
      allRouteDetails[route.currentSeqNo].routedetailid,
      route.currentSeqNo,
      allRouteDetails[route.currentSeqNo].statusid,
      route.lastUpdatedUserId,
    );
...
const getRoute = async (req, res) => {
  try {
    ...
    for await (let row of routeInfos) {
      const routeDetails = await saveRouteModel.getRouteDetailsByRouteId(
        row.routeid,
      );
      const routeStatus = await saveRouteModel.getRouteStatusByRouteId(
        row.routeid,
      );
