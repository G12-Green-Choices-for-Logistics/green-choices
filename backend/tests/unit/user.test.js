...
describe("User Model", () => {
  ...
  describe("updateUser", () => {
    ...
    it("should handle database errors during update", async () => {
      ...
      // Call function and expect it to throw
      await expect(userModel.updateUser(userId, userData)).rejects.toThrow(
        "Database error",
      );
    });
  });
  ...
  describe("getUserById", () => {
    ...
    it("should handle database errors when getting user by ID", async () => {
      ...
      // Call function and expect it to throw
      await expect(userModel.getUserById(userId)).rejects.toThrow(
        "Database error",
      );
    });
  });
  ...
  describe("getUserAuditHistory", () => {
    it("should get user audit history successfully", async () => {
      ...
      // Assertions
      expect(db.query).toHaveBeenCalledWith(
        "SELECT * FROM fn_GetUserAuditHistory($1)",
        [userId],
      );
    ...
    it("should handle database errors when getting audit history", async () => {
      ...
      // Call function and expect it to throw
      await expect(userModel.getUserAuditHistory(userId)).rejects.toThrow(
        "Database error",
      );
    });
  });
