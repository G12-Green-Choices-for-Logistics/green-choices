  ...
  "New York, USA": [
    {
      ...
      emission_reason: [
        "",
        "Long distance air travel causes a lot of emissions",
      ],
    },
    {
  ...
  "London, UK": [
    {
      ...
      emission_reason: [
        "",
        "For shorter flights, taking off causes the most emissions",
      ],
    },
  ],
  "Paris, France": [
...
describe("Route Model", () => {
  ...
  describe("findAllRoutes", () => {
    ...
    test("should respect maxRoutes parameter", () => {
      const maxRoutes = 1;
      const routes = routeModel.findAllRoutes(
        "New York, USA",
        "Rome, Italy",
        maxRoutes,
      );
    ...
    test("should not enter infinite loop with cyclic routes", () => {
      ...
      expect(routes.length).toBeGreaterThan(0);
      expect(() =>
        routeModel.findAllRoutes("London, UK", "Paris, France"),
      ).not.toThrow();
    });
  });
  ...
  describe("getRouteEmissionReasons", () => {
    test("should return reasons for high emissions due to long distance", () => {
      ...
      expect(reasons.minRouteReasons).toContain(
        "Long distance routes aren't feasible to have low carbon emissions",
      );
      expect(reasons.maxRouteReasons).toContain(
        "Long distance routes aren't feasible to have low carbon emissions",
      );
    });
