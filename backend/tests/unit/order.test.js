...
describe("Order Model", () => {
  ...
  describe("createOrder", () => {
    const mockOrderData = {
      ...
      orderItems: [
        ...
        { productId: 2, quantity: 1, price: 50.5 },
      ],
    };
  ...
  describe("updateOrder", () => {
    ...
    test("should throw an error if database query fails", async () => {
      ...
      await expect(
        orderModel.updateOrder(orderId, mockOrderData),
  ...
  describe("getOrderById", () => {
    ...
    test("should throw an error if database query fails", async () => {
      ...
      await expect(orderModel.getOrderById(orderId)).rejects.toThrow(
        "Database error",
      );
  ...
  describe("getAllOrders", () => {
    test("should get all orders successfully", async () => {
      const mockOrders = [
        { orderid: 1, userid: 1, totalamount: 100.5 },
  ...
  describe("getOrdersByUserId", () => {
    ...
    test("should get orders by user ID successfully", async () => {
      const mockOrders = [
        { orderid: 1, userid: userId, totalamount: 100.5 },
      ...
      expect(db.query).toHaveBeenCalledWith(
        "SELECT * FROM fn_GetOrdersByUserID($1)",
        [userId],
      );
      expect(result).toEqual(mockOrders);
    });
    ...
    test("should throw an error if database query fails", async () => {
      ...
      await expect(orderModel.getOrdersByUserId(userId)).rejects.toThrow(
        "Database error",
      );
  ...
  describe("getActiveOrdersByUserId", () => {
    ...
    test("should throw an error if database query fails", async () => {
      ...
      await expect(orderModel.getActiveOrdersByUserId(userId)).rejects.toThrow(
        "Database error",
      );
  ...
  describe("getOrderAuditHistory", () => {
    ...
    test("should get order audit history successfully", async () => {
      ...
      expect(db.query).toHaveBeenCalledWith(
        "SELECT * FROM fn_GetOrderAuditHistory($1)",
        [orderId],
      );
    ...
    test("should throw an error if database query fails", async () => {
      ...
      await expect(orderModel.getOrderAuditHistory(orderId)).rejects.toThrow(
        "Database error",
      );
