...
describe("Save Route Controller", () => {
  ...
  describe("updateRoute", () => {
    ...
    test("should handle case where new segments are added during update", async () => {
      ...
      saveRouteModel.saveRouteDetails.mockResolvedValueOnce({
        routedetailid: 2,
        routeid: 1,
        seqno: 1,
        statusid: 1,
  ...
  describe("getRoute", () => {
    test("should get routes by order id and return 200 status", async () => {
      ...
      const mockRouteDetails2 = [{ routedetailid: 3, routeid: 2, seqno: 0 }];
      ...
      expect(saveRouteModel.getRouteByOrderId).toHaveBeenCalledWith(
        req.query.orderId,
      );
