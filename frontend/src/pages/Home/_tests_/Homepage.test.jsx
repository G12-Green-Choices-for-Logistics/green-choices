...
describe("Homepage", () => {
  ...
  it("Should display top section", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByRole("heading", { level: 1, name: /FLASH SALE!!/ }),
      ).toBeInTheDocument();
    });
  });
  ...
  it("Should display products and categories", async () => {
    ...
    await waitFor(async () => {
      const trendingHeading = await screen.findByRole("heading", {
        name: /Trending Now/,
      });
      const bestDealsHeading = await screen.findByRole("heading", {
        name: /Best Deals/,
      });
  ...
  it("Should contain product details", async () => {
    ...
    await waitFor(async () => {
      ...
      trendingArray.forEach((product, index) => {
        ...
        expect(utils.getByRole("img")).toHaveAttribute(
          "src",
          mockProducts[index].img,
        );
        expect(utils.getByRole("heading", { level: 3 })).toHaveTextContent(
          mockProducts[index].name,
        );
        expect(utils.getByRole("paragraph")).toHaveTextContent(
  ...
  it("Should open up product modal", async () => {
    ...
    await waitFor(async () => {
      ...
      expect(
        within(modal).getByAltText("Caliber Canvas Suede Shoes"),
      ).toHaveAttribute("src", mockProducts[0].img);
      expect(
        within(modal).getByRole("heading", { level: 1 }),
      ).toHaveTextContent(mockProducts[0].name);
      expect(
        within(modal).getByText(`$ ${mockProducts[0].price}`),
      ).toBeInTheDocument();
    });
  });
  ...
  it("Should close modal when X is pressed", async () => {
    ...
    await waitFor(async () => {
      ...
      const button = within(modal).getByRole("button", {
        name: /close product quick view/i,
      });
  ...
  it("Should be able to redirect to other correct pages", async () => {
    ...
    await waitFor(async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByRole("heading", { level: 1, name: /Active Orders/ }),
        ).toBeInTheDocument();
      });
