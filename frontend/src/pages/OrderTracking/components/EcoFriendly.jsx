...
const EcoFriendly = ({
  ...
  const variants = {
    choice: {
      ...
      defaultDescription:
        "Would you like to switch to a more sustainable delivery method?",
    ...
    confirmation: {
      ...
      defaultDescription:
        "You've selected the most eco-friendly delivery option",
  ...
  return (
    <div className="relative overflow-hidden rounded-xl border border-green-200 bg-white p-4 shadow-sm transition-all duration-200 hover:shadow-md sm:p-6">
      ...
      <div className="relative z-10 max-w-[80%] sm:max-w-[70%] flex flex-col gap-3">
        <h2
          className={`text-lg font-bold sm:text-xl md:text-2xl ${currentVariant.titleColor}`}
        >
        ...
        <div className="flex items-center gap-4 mt-2">
          {variant === "choice" ? (
            ...
            <p className="text-sm text-gray-600 sm:text-base">
              {secondaryText ||
                "Your choice is helping to reduce carbon emissions and support a greener planet."}
              {onSecondaryAction && (
                <button
